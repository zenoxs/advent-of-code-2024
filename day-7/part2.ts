...
function concatenateNumbers(a: number, b: number, base = 10) {
...
function getPotentialResults(equation: Equation,  numberIndex: number = 0): number[] {
  ...
  const concats = getPotentialResults(equation, numberIndex + 1).map(prevValue => {
    return concatenateNumbers(prevValue, currentNumber)
  })

  return [...sums, ...mults, ...concats]
}
